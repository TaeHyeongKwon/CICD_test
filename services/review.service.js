...
const reviewValidate = require('../validations/reviewValidate');
const imagesValidate = require('../validations/imagesValidate');
...
class ReviewService {
  ...
  createReview = async (
    ...
    try {
      ...
      if (administrativeDistrict.includes(doCityDong[0])) {
        await this.reviewRepository.createDo(
          doCityDong[0],
          await addressToGeO(doCityDong[0]),
        );
        await this.reviewRepository.createCity(
          doCityDong[0],
          await addressToGeO(doCityDong[0]),
        );
        await this.reviewRepository.createDong(
          doCityDong[2],
          await addressToGeO(doCityDong[2]),
        );
      } else {
        await this.reviewRepository.createDo(
          doCityDong[0],
        ...
        await this.reviewRepository.createCity(
          doCityDong[1],
          await addressToGeO(doCityDong[1]),
        );
        await this.reviewRepository.createDong(
          doCityDong[2],
          await addressToGeO(doCityDong[2]),
      ...
      if (reviewExist) {
        ...
        let estate = await this.reviewRepository.createEstate(
          address,
          address_jibun,
          await clustering(address),
        );
        let review = await this.reviewRepository.createReview(
          estate.estateId,
          // nickname,
  ...
  getReview = async (estateId) => {
    const reviewEstateInfoEstate = await this.reviewRepository.getReview(
      estateId,
    );
